...
async function main() {
    ...
    const entries = Object.entries(models).filter(([, value]) => value instanceof mongoose.Model || value?.prototype instanceof mongoose.Document);
    ...
    for (const [name, model] of entries) {
        ...
        const wanted = new Set(schemaIndexes.map(([key]) => JSON.stringify(key)));
        const dropped = [];
