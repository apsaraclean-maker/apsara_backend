...
async function main() {
    ...
    if (allowOrigin === ORIGIN && allowCreds === 'true')
        ok(`${pre.status}, allow-origin=${allowOrigin}, allow-credentials=${allowCreds}`);
    else
        bad(`${pre.status}, allow-origin=${allowOrigin}, allow-credentials=${allowCreds} — ${ORIGIN} must be in ALLOWED_ORIGINS (server.ts)`);
    // ── 3. login ──────────────────────────────────────────────────────────────
    ...
    if (login.ok)
        ok(`${login.status} ${body.slice(0, 80)}…`);
    else
        bad(`${login.status} ${body.slice(0, 220)}`);
    const setCookie = login.headers.get('set-cookie');
    if (setCookie)
        ok(`Set-Cookie issued: ${setCookie.split(';')[0].slice(0, 40)}… [${setCookie.split(';').slice(1).map((s) => s.trim()).join(', ')}]`);
    else
        bad('no Set-Cookie on the login response');
    // ── 4. cookie round trip ──────────────────────────────────────────────────
    if (setCookie) {
        ...
        if (me.ok)
            ok(`${me.status} — the server-side flow is healthy end to end`);
        else
            bad(`${me.status} — session did not survive the round trip`);
    }
