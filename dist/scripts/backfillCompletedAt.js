...
async function main() {
    ...
    for (const entry of history) {
        ...
        if (!completedAtByOrder.has(key))
            completedAtByOrder.set(key, entry.changed_at);
    }
    let resolved = 0;
    ...
    for (const order of candidates) {
        ...
        resolved += 1;
        if (order.delivery_due_date && completedAt.getTime() > order.delivery_due_date.getTime())
            wouldBeDelayed += 1;
