...
router.get('/businesses', adminAuthMiddleware, async (_req, res) => {
    try {
        ...
        const result = businesses.map((biz) => {
            ...
            const paymentLabel = biz.status !== 'active' ? 'N/A' : paidBusinessIds.has(String(biz._id)) ? 'Paid' : 'Delayed';
            return {
                _id: biz._id,
                name: biz.name,
                phone: biz.phone,
...
router.put('/payments/:id', adminAuthMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (notes !== undefined)
            payment.notes = notes;
        if (cycle_start_date)
            payment.cycle_start_date = cycle_start_date;
        if (cycle_end_date)
