...
router.get('/sales', authorizeRoles('owner', 'manager'), async (req, res) => {
    try {
        ...
        if (duration === 'custom') {
            ...
            const windowDays = DURATION_DAYS[duration] || DURATION_DAYS.monthly;
        ...
        const paidDateByOrder = new Map();
        for (const h of paidHistory) {
            const key = String(h.order_id);
            if (!paidDateByOrder.has(key))
                paidDateByOrder.set(key, h.changed_at); // sorted desc — first hit is latest
        }
