...
router.get('/', async (req, res) => {
    try {
        ...
        const query = { business_id: req.user.businessId, deleted_at: null };
        if (active_only === 'true')
        ...
        if (washing_method)
            query.washing_method = washing_method;
        ...
        const priceBound = {};
        if (min_price)
            priceBound.$gte = Number(min_price);
        if (max_price)
            priceBound.$lte = Number(max_price);
        ...
        if (branch_id) {
            const links = await BranchService.find({ branch_id: new mongoose.Types.ObjectId(branch_id) })
                .select('service_id')
                .lean();
        ...
        for (const link of allLinks) {
            ...
            if (!linksByService.has(key))
                linksByService.set(key, []);
...
router.post('/', authorizeRoles('owner'), async (req, res) => {
    ...
    try {
        ...
        await BranchService.insertMany(branch_ids.map((bid) => ({ branch_id: bid, service_id: service._id })));
...
router.put('/:id', authorizeRoles('owner'), async (req, res) => {
    ...
    try {
        ...
        if (article_type !== undefined)
            service.article_type = article_type;
        if (washing_method !== undefined)
            service.washing_method = washing_method;
        if (unit_price !== undefined)
            service.unit_price = Number(unit_price);
        if (weight_price !== undefined)
            service.weight_price = Number(weight_price);
        if (notes !== undefined)
            service.notes = notes;
...
router.patch('/:id/toggle', authorizeRoles('owner', 'manager'), async (req, res) => {
...
async function serveMaster(key, load, res) {
