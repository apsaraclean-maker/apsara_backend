...
import { BILLING_GRACE_DAYS, countBillableOrders, countBillableOrdersByCycle, getBillingState, } from '../utils/billing.js';
import { computeBill, getCycleByIndex, getCycleIndexAt, } from '../utils/billingCycle.js';
...
router.get('/overview', async (req, res) => {
    try {
        ...
        const branchId = requestedBranch && (!accessible || accessible.includes(requestedBranch)) ? requestedBranch : null;
        ...
        const [branchOrders, businessOrders] = await Promise.all([
            branchId ? countBillableOrders(business._id, cycle, branchId) : Promise.resolve(null),
...
router.get('/payments', async (req, res) => {
    try {
        ...
        const [counts, strayCounts] = await Promise.all([
            ...
            Promise.all(strays.map(async (p) => {
                const adHoc = { index: -1, cycleStart: p.cycle_start_date, cycleEnd: p.cycle_end_date };
                return [p.cycle_start_date, await countBillableOrders(business._id, adHoc)];
            })),
        ]);
        for (const [key, n] of strayCounts)
            counts.set(key, n);
        res.json({
