...
async function hasOrderBranchAccess(user, order) {
...
const storage = multer.diskStorage({
    destination: (req, _file, cb) => {
        ...
        if (!businessId)
            return cb(new Error('Business ID missing'), '');
...
router.post('/rate/:token', async (req, res) => {
    ...
    const isValidRating = (v) => v === undefined || (Number.isInteger(v) && v >= 1 && v <= 5);
...
function isValidStatusTransition(from, to) {
    if (from === to)
        return false;
    if (from === 'cancelled')
        return to === 'created'; // reopen
    if (to === 'cancelled')
        return from === 'created' || from === 'in_progress'; // cancel
...
router.get('/', async (req, res) => {
    try {
        const { status, branch_id, start_date, end_date, search, is_delayed, service_id, min_price, max_price, created_by, timeline_date, timeline_status, include_history, page = '1', limit = '20', sort = 'created_desc', } = req.query;
        ...
        const match = { business_id: new mongoose.Types.ObjectId(req.user.businessId), deleted_at: null };
        if (status)
            match.status = status;
        if (created_by)
        ...
        if (min_price || max_price) {
            match.total_price = {};
            if (min_price)
                match.total_price.$gte = Number(min_price);
            if (max_price)
        ...
        if (accessibleBranchIds === null) {
            ...
            if (!accessibleBranchIds.includes(branch_id)) {
        ...
        if (timelineMode) {
            ...
            if (ts === 'created') {
                ...
                if (ts === 'completed')
                    ...
                    match.status = ts; // paid → 'paid'; cancelled → 'cancelled' (terminal)
            }
        }
...
// PUT /api/orders/:id
router.put('/:id', authorizeRoles('owner', 'manager'), async (req, res) => {
    ...
    try {
        ...
        if (delivery_due_date !== undefined)
            order.delivery_due_date = delivery_due_date;
        if (extra_charges !== undefined)
            order.extra_charges = Number(extra_charges);
        if (extra_charges_reason !== undefined)
        ...
        if (items !== undefined) {
            ...
            if (orderItems.length)
                await OrderService.insertMany(orderItems);
...
router.patch('/:id/status', authorizeRoles('owner', 'manager'), async (req, res) => {
...
router.patch('/:id/notes', authorizeRoles('owner', 'manager'), async (req, res) => {
...
router.post('/:id/images', authorizeRoles('owner', 'manager'), upload.array('images', 5), async (req, res) => {
    try {
        const order = await Order.findOne({ _id: req.params.id, business_id: req.user.businessId, deleted_at: null });
        if (!order)
            return res.status(404).json({ message: 'Order not found' });
...
router.delete('/:id/images/:imageId', authorizeRoles('owner', 'manager'), async (req, res) => {
    try {
        const order = await Order.findOne({ _id: req.params.id, business_id: req.user.businessId, deleted_at: null }).select('branch_id');
        if (!order)
            return res.status(404).json({ message: 'Order not found' });
