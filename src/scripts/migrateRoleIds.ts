...
const DEFAULT_MAPPING: Record<string, string> = { '1': 'owner', '2': 'manager', '3': 'worker', '0': 'admin' };
...
async function main() {
  ...
  const updates: { _id: mongoose.Types.ObjectId; from: any; to: string | null }[] = [];
  for (const doc of candidates) {
    const roleId = doc.roleId !== undefined ? String(doc.roleId) : undefined;
    const mapped = roleId !== undefined ? mapping[roleId] ?? null : null;
    console.log(
      `  _id=${doc._id}  phone=${doc.phone ?? '?'}  roleId=${doc.roleId ?? '—'}  role=${doc.role ?? '—'}  -> ${mapped ?? 'UNRESOLVED (needs manual review)'}`
    );
