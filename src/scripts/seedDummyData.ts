...
function getSeededOrderNumber(branchId: string, branchCode: string, employeeId: string, date: DateTime): string {
...
function nextDummyPhone(): string {
...
async function scrapLegacyRoleIdStaff() {
  ...
  if (userIds.length) await UserBranch.deleteMany({ user_id: { $in: userIds } });
...
async function seedOneBusiness(template: typeof BUSINESS_TEMPLATES[number], homeCity: typeof CITIES[number]) {
  ...
  const statusPlan: Array<'created' | 'in_progress' | 'completed' | 'paid' | 'cancelled'> = [];
  for (let i = 0; i < orderCount; i++) {
    const r = i / orderCount;
    // Weighted spread: mostly paid/completed (realistic history), some in-flight, a few cancelled.
    if (r < 0.45) statusPlan.push('paid');
    else if (r < 0.65) statusPlan.push('completed');
    else if (r < 0.8) statusPlan.push('in_progress');
    else if (r < 0.92) statusPlan.push('created');
  ...
  for (let i = 0; i < orderCount; i++) {
    ...
    const progression: Array<'created' | 'in_progress' | 'completed' | 'paid'> =
