...
router.put('/profile', authorizeRoles('owner'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (social_link !== undefined) business.social_link = social_link;
    if (website !== undefined) business.website = website;
    if (facebook_url !== undefined) business.facebook_url = facebook_url;
    if (instagram_url !== undefined) business.instagram_url = instagram_url;
