...
export function isOrderDelayed(order: { is_delayed?: boolean; delivery_due_date?: string | null; status: string }): boolean {
  if (order.is_delayed) return true;
  if (!order.delivery_due_date) return false;
  if (NON_DELAYABLE_STATUSES.includes(order.status)) return false;
